...
var cmdMap = map[string]command{
	"server": {
		desc: "Backoffice web server which faculty log into in order to associate GEs/GTFs with courses",
		run:  sciserver.Run,
	},
	"import-csv": {
	...
	"export-csv": {
		desc: "CSV exporter for sending list of GEs/GTFs out to canvas",
		run:  exportcsv.Run,
	},
}
